...
#  TODO: Use this code for the monotonic version and confirm that it converges close to sklearn
#  TODO: Find a way to normalise the mixture (currently weights do not sum to 1)
#  TODO: Implement with new normalisation scheme
#  TODO: Test that the mixture indeed is a valid pdf
# ---------------------------------------------------------------------------

class NMGaussianMixture(nn.Module, HookTensorBoard, HookVisualiseGMM):
    ...
    def __init__(self, n_clusters: int, n_dims: int):
        ...
        self.mu = nn.ParameterList(
            nn.Parameter(torch.zeros(n_dims, dtype=torch.float64).normal_()) for _ in range(n_clusters))
    ...
    def pdf(self, X: torch.Tensor) -> torch.Tensor:
        ...
        if ((norm * pdf) < 0).any(): print("LESS THAN 0 INVALID")
        if ((norm * pdf) > 1).any(): print("MORE THAN 1 INVALID")
